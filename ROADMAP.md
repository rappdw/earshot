...
| C | Timestamp-drift diagnostic | Whisper timestamps observed drifting from audio; that skews diarize speaker assignment, not just clips. Ship `earshot timing DIR` to measure segment-vs-turn alignment | synthetic data here; real meeting run by Dan → transcribe fix if confirmed |
